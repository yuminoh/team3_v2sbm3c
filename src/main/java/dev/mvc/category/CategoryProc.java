...
public class CategoryProc implements CategoryProcInter {
	...
	public List<CategoryVO> category_list(HashMap<String, Object> map){
	     ...
	     */
	    int begin_of_page = ((Integer) map.get("now_page") - 1) * Category.RECORD_PER_PAGE;
	    ...
	    // 2 페이지 = 0 + 20: 20
	    // 3 페이지 = 0 + 30: 30
	    int end_num = begin_of_page + Category.RECORD_PER_PAGE;
	    /*
		...
		List<CategoryVO> list = this.CategoryDAO.category_list(map);
		return list;
	};
	...
	public List<CategoryVO> category_list_data(){
		List<CategoryVO> list = this.CategoryDAO.category_list_data();
		return list;
	};
	...
	@Override
	  public String pagingBox(int search_count, int now_page, String word) {
	      int total_page = (int) (Math.ceil((double) search_count / Category.RECORD_PER_PAGE)); // 전체 페이지 수
	      int total_grp = (int) (Math.ceil((double) total_page / Category.PAGE_PER_BLOCK)); // 전체 그룹 수
	      int now_grp = (int) (Math.ceil((double) now_page / Category.PAGE_PER_BLOCK)); // 현재 그룹 번호

	      // 1 group: 1, 2, 3 ... 9, 10
	      // 2 group: 11, 12 ... 19, 20
	      // 3 group: 21, 22 ... 29, 30
	      int start_page = ((now_grp - 1) * Category.PAGE_PER_BLOCK) + 1; // 특정 그룹의 시작 페이지
	      int end_page = (now_grp * Category.PAGE_PER_BLOCK); // 특정 그룹의 마지막 페이지
...
//	    str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
	      ...
	      int _now_page = (now_grp - 1) * Category.PAGE_PER_BLOCK;
	      if (now_grp >= 2) { // 현재 그룹번호가 2이상이면 페이지수가 11페이 이상임으로 이전 그룹으로 갈수 있는 링크 생성
	          str.append("<span class='span_box_1'><A href='" + Category.LIST_FILE + "?&word=" + word + "&now_page="
	                  + _now_page +  "'>이전</A></span>");
	      }
	      ...
	      _now_page = (now_grp * Category.PAGE_PER_BLOCK) + 1; // 최대 페이지수 + 1
	...
	public int search_count(HashMap<String, Object> map) {
		int cnt = this.CategoryDAO.search_count(map);
		return cnt;
	};
}
