...
<script type="text/javascript">
  ...
  function update_recom_ajax(noticeno) {
    ...
    $('#span_animation').html("<img src='/notice/images/ani04.gif' style='width: 8%;'>");
  ...
  <%-- 쇼핑 카트에 상품 추가 --%>
  function cart_ajax(noticeno) {
    var f = $('#frm_login');
    $('#noticeno', f).val(noticeno);  // 쇼핑카트 등록시 사용할 상품 번호를 저장.
    
    console.log('-> noticeno: ' + $('#noticeno', f).val()); 
  ...
  function cart_ajax_post() {
    var f = $('#frm_login');
    var noticeno = $('#noticeno', f).val();  // 쇼핑카트 등록시 사용할 상품 번호.
...
<DIV class='content_body'>
  <ASIDE class="aside_right">
    ...
    <span class='menu_divide' >│</span>
    <A href="./update_text.do?noticeno=${noticeno}&now_page=${param.now_page}">수정</A>
    <span class='menu_divide' >│</span>
    <A href="./update_file.do?noticeno=${noticeno}&now_page=${param.now_page}">파일 수정</A>  
    <span class='menu_divide' >│</span>
    <A href="./delete.do?noticeno=${noticeno}&now_page=${param.now_page}&cateno=${cateno}">삭제</A>  
  </ASIDE> 
        ...
        <DIV style="width: 50%; float: left; margin-right: 10px;">
            <c:choose>
              ...
              <c:otherwise> <!-- 기본 이미지 출력 -->
                <IMG src="/notice/images/none1.png" style="width: 100%;"> 
              </c:otherwise>
            </c:choose>
        </DIV>
