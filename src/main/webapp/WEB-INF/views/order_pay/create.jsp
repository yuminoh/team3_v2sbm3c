...
<script type="text/javascript">
  ...
  function my_address() {
    ...
    $.ajax({
      ...
      success: function(rdata) { // 서버로부터 성공적으로 응답이 온경우
        ...
        $('#rc_name').val(rdata.rname);
        $('#rc_tel').val(rdata.rtel);
        $('#rc_zipcode').val(rdata.rzipcode);
        $('#rc_address1').val(rdata.raddress1);
        $('#rc_address2').val(rdata.raddress2);
  ...
  function send() {
    if (check_null($('#rc_name').val()))  {
      alert('수취인 성명을 입력해주세요.');
      $('#rc_name').focus();
      return;
    } 

    if (check_null($('#rc_tel').val()))  {
      alert('수취인 전화번호를 입력해주세요.');
      $('#rc_tel').focus();
      return;
    } 

    if (check_null($('#rc_zipcode').val()))  {
      alert('우편번호를 입력해주세요.');
      $('#rc_zipcode').focus();
      return;
    }    

    if (check_null($('#rc_address1').val()))  {
      alert('주소를 입력해주세요.');
      $('#rc_address1').focus();
      return;
    }     
     
    if (check_null($('#rc_address2').val()))  {
...
<br>
  ...
  <table class="table table-striped" style='width: 100%;'>
    ...
    <tbody>
      <c:forEach var="cartVO" items="${list }">
        ...
        <c:set var="pdimagefile1" value="${cartVO.pdimagefile1 }" />              
        <tr> 
            ...
            <c:choose>
              <c:when test="${pdimagefile1.endsWith('jpg') || pdimagefile1.endsWith('png') || pdimagefile1.endsWith('gif')}">
                <%-- /static/contents/storage/ --%>
                 <a href="/products/read.do?productno=${productno}"><IMG src="/products/storage/${pdimagefile1 }" style="width: 120px; height: 80px;"></a> 
              </c:when>
          ...
          <td style='vertical-align: middle; text-align: center;'>
            <fmt:formatNumber value="${cnt * product_price}" pattern="#,###" />
          </td>
          <td style='vertical-align: middle; text-align: center;'>
            <A href="../cart/list_by_memberno"><IMG style="width:30px; height:30px" src="/cart/images/delete3.png" title="쇼핑카트로 이동합니다."></A>
          </td>
        </tr>
      </c:forEach>
  ...
  <form name='frm' id='frm' style='margin-top: 50px;' action="/order_pay/create.do" method='post'>
    ...
    <input type="hidden" name="amount_pay" value=" ${total_order }">   <%-- 전체 주문 금액 --%>
...
<br>
  <div class='menu_line'></div>
    ...
    <div class="form-group">
      ...
      <div class="col-md-10">
        <input type='text' class="form-control" name='rc_tel' id='rc_tel' 
    ...
    <div class="form-group">
      ...
      <div class="col-md-10">
        <input type='text' class="form-control" name='rc_zipcode' id='rc_zipcode' 
    ...
    <div class="form-group">
      ...
      <div class="col-md-10">
        <input type='text' class="form-control" name='rc_address1' id='rc_address1' 
    ...
    <div class="form-group">
      ...
      <div class="col-md-10">
        <input type='text' class="form-control" name='rc_address2' id='rc_address2' 
