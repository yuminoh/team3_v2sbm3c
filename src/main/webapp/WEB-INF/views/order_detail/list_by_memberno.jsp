...
<jsp:include page="../menu/top.jsp" flush='false' />
  ...
  <DIV class='content_body' style='width: 100%;'>
    ...
    <c:forEach var="order_detailVO" items="${list }">
      <c:set var="payno" value ="${order_detailVO.payno}" />
      <c:set var="order_itemno" value ="${order_detailVO.order_itemno}" />
      <c:set var="memberno" value ="${order_detailVO.memberno}" />
      <c:set var="productno" value ="${order_detailVO.productno}" />
      <c:set var="product_Explanation" value ="${order_detailVO.product_Explanation}" />
      <c:set var="product_price" value ="${order_detailVO.product_price}" />
      <c:set var="cnt" value ="${order_detailVO.cnt}" />
      <c:set var="tot" value ="${order_detailVO.tot}" />
      <c:set var="stateno" value ="${order_detailVO.stateno}" />
      <c:set var="rdate" value ="${order_detailVO.rdate}" />
         
    <TR>
      <TD class=td_basic>${payno}</TD>
      <TD class=td_basic>${order_itemno}</TD>
      <TD class=td_basic>${memberno}</TD>
      <TD class=td_basic><A href="/products/read?productno=${productno}">${productno}</A></TD>
      <TD class='td_left'>${product_Explanation}</TD>
      <TD class='td_left'><fmt:formatNumber value="${product_price }" pattern="#,###" /></TD>
